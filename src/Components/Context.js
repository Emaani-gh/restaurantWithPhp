...
export const Provider = (props) => {
  ...
  const fetchData = (param, setArr) => {
    fetch(`http://localhost:8000/public/data.php?${param}`)
      .then((res) => res.json())
  ...
  const calcExtras = (e) => {
    ...
    if (isChecked) {
      setCurrentProduct({
        ...currentProduct,
        setSumExtra:
          Math.round((currentProduct.setSumExtra + value) * 1e2) / 1e2,
      });
    } else {
      setCurrentProduct({
        ...currentProduct,
  ...
  const addToCart = (product) => {
    const isFound = cart.find(
      (ele) =>
        ele.prd_id === product.prd_id &&
        ele.selectedPrice === product.selectedPrice
    );
    if (isFound) {
      setCart(
        cart.map((prd) =>
          prd === isFound ? { ...isFound, qtn: isFound.qtn + 1 } : prd
        )
      );
    } else {
  ...
  const removeFromCart = (product) => {
    ...
    if (found.qtn === 1) {
      ...
      setCart(
        cart.map((prd) =>
          prd.prd_id === product.prd_id &&
          prd.selectedPrice === product.selectedPrice
            ? { ...prd, qtn: prd.qtn - 1 }
            : prd
        )
      );
